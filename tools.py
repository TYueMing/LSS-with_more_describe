...
# 实现了一种快速的累加操作（cumulative sum），并对输入张量进行筛选和处理
class QuickCumsum(torch.autograd.Function):
    # 继承了 torch.autograd.Function，表示它是一个自定义的自动微分函数，用于自定义张量操作及其梯度计算
    @staticmethod
    def forward(ctx, x, geom_feats, ranks):
        # ctx：上下文对象，用于存储反向传播中需要的中间结果
        # x：输入张量，执行累加操作的目标
        # geom_feats：几何特征张量，不需要梯度
        # ranks：排序信息，用于筛选累加结果
        x = x.cumsum(0)   # x.cumsum(0)：对输入张量 x 按第 0 维进行累加操作
        kept = torch.ones(x.shape[0], device=x.device, dtype=torch.bool)  # kept：创建一个布尔张量，表示保留的元素索引
        kept[:-1] = (ranks[1:] != ranks[:-1])  # kept[:-1] = (ranks[1:] != ranks[:-1])：比较相邻的 ranks 值，仅在相邻值不相等时保留对应的元素

        x, geom_feats = x[kept], geom_feats[kept]
        x = torch.cat((x[:1], x[1:] - x[:-1]))
        # 将第一个元素与相邻元素的差值拼接起来，计算“累加变化”

        # save kept for backward  ctx.save_for_backward(kept)：保存 kept 张量以供反向传播使用
        ctx.save_for_backward(kept)

        # no gradient for geom_feats   标记 geom_feats 为不可微分的张量
